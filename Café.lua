--========================================================--
--                  CAFEÍNA V1.0
--        MOBILE / ROBLOX STUDIO / LOCALScript
--========================================================--
-- PRINCIPAL
-- MOVIMENTO
--   ├ NOCLIP
...
-- Esta interface precisa rodar no cliente (LocalScript).
if not RunService:IsClient() then
    warn("CAFEÍNA V1: execute este arquivo como LocalScript no cliente.")
    return
end
...
if not Player then
    warn("CAFEÍNA V1: LocalPlayer ainda não está disponível.")
    return
end
...
if not PlayerGui then
    warn("CAFEÍNA V1: PlayerGui não foi encontrado.")
    return
end
...
local function CreateToggle(
    ...
    -- Estado local por toggle. Evita colisões quando dois controles
    -- possuem o mesmo texto (ex.: vários botões chamados "Ativar").
    local CurrentState = Default
    States[Toggle] = Default
        ...
        Connect(
            function()
                ...
                if Move.Magnitude > .05 then
                    ...
                    -- MoveDirection já representa o analógico/teclado no mundo.
                    -- Usamos a direção horizontal para saber frente/lado e
                    -- a inclinação da câmera para permitir subir/descer.
                    local FlatLook =
                        Vector3.new(
                    ...
                    if FlatRight.Magnitude > .001 then
                    ...
                    -- Ao avançar/retroceder, acompanha a inclinação da câmera.
                    Direction =
                        Direction
                        + Vector3.new(
                            0,
                    ...
                    if Direction.Magnitude > .05 then
    ...
    AimConnection =
        ...
        Connect(
            function()
                ...
                -- Suavização baseada no delta ajuda a ficar consistente
                -- em aparelhos com FPS diferentes.
                Camera.CFrame =
                    Current:
                    Lerp(
                        Desired,
...
--========================================================--
-- MOUNTAIN LAB • MOTOR FUNCIONAL PARA ROBLOX STUDIO
--========================================================--
-- As rotinas abaixo são deliberadamente limitadas ao Studio.
-- Elas permitem validar seu sistema de mineração/coleta/venda
-- usando tags, nomes e RemoteEvents/RemoteFunctions do seu jogo.
--========================================================--
...
local MountainHighlights = {}
local MountainLoopsStarted = false
...
local SellRemoteWords = {
    "sell",
    "vender",
    "sale",
}

local function StudioMountainAllowed()
    ...
    Notify(
        "Mountain Lab",
        "Estas rotinas funcionais estão disponíveis somente no Roblox Studio."
    )
...
local function LowerName(Object)
    return string.lower(
        tostring(Object and Object.Name or "")
    )
end
...
local function IsCandidateByTag(
    Object,
    Tags
)
...
local function IsCrystal(Object)
    if not Object then
        return false
    end

    return IsCandidateByTag(
        Object,
        CrystalTags
    )
    or HasWord(
...
local function GetNearestMountainTarget(
    Kind
)
    if not Root then
        return nil
    ...
    local Best = nil
    local BestDistance =
        MountainState.DetectionRadius

    for _, Object in ipairs(
        CollectCandidates(Kind)
    ) do
        local Position =
...
local function ClearMountainHighlight(
    Object
)
    local Highlight =
        MountainHighlights[Object]
    ...
    if Highlight then
        ...
        MountainHighlights[Object] = nil
    end
end

local function RefreshMountainHighlights()
    local Wanted = {}
    ...
    if MountainState.CrystalESP then
        for _, Object in ipairs(
            CollectCandidates("Crystal")
        ) do
            Apply(Object, "CRYSTAL")
        end
    end

    if MountainState.BoulderESP then
        for _, Object in ipairs(
            CollectCandidates("Boulder")
        ) do
            Apply(Object, "BOULDER")
        end
    end
    ...
    for Object in pairs(
        ...
        if not Wanted[Object]
            or not Object.Parent then
            ClearMountainHighlight(Object)
        end
    end
end

local function FindRemoteByWords(
    Words
)
    for _, Object in ipairs(
        ReplicatedStorage:
...
local function CallMountainRemote(
    Remote,
    ...
)
    if not Remote then
...
local function TeleportToMountainTarget(
    Kind
)
    if not StudioMountainAllowed() then
        return false
    end
    ...
    local Target =
        GetNearestMountainTarget(Kind)

    if not Target then
        Notify(
            "Mountain Lab",
            "Nenhum alvo encontrado no raio configurado."
        )
        return false
    end
...
local function MountainPickupStep()
    if not StudioMountainAllowed() then
        return
    end

    local Target =
        GetNearestMountainTarget(
            "Crystal"
        )
...
local function MountainFarmStep()
    if not StudioMountainAllowed() then
        return
    end
...
local function MountainSellStep()
    if not StudioMountainAllowed() then
        return
    end
    ...
    if Remote then
        ...
        if not Ok then
            CallMountainRemote(Remote)
        end
    end
end

local function StartMountainLoops()
    if MountainLoopsStarted then
        return
    end

    MountainLoopsStarted = true

    task.spawn(function()
        while Gui and Gui.Parent do
            if MountainState.CrystalESP
                or MountainState.BoulderESP then
                RefreshMountainHighlights()
            elseif next(MountainHighlights) then
                RefreshMountainHighlights()
            end
        ...
        while Gui and Gui.Parent do
            local Delay =
                math.max(
                    1,
                    tonumber(
                        MountainState.Interval
                    ) or 5
                )

            if RunService:IsStudio() then
                if MountainState.Teleport then
                    TeleportToMountainTarget(
                        "Crystal"
                    )
                end

                if MountainState.Pickup then
                    MountainPickupStep()
                end

                if MountainState.Farm then
                    MountainFarmStep()
                end

                if MountainState.Sell then
                    MountainSellStep()
                end
            end
...
--========================================================--
-- MOUNTAIN LAB
-- Estrutura inspirada nas áreas do script analisado,
-- mantendo o layout visual do CAFEÍNA V1.
-- Funções de automação/exploit foram substituídas por
-- controles seguros de laboratório para Roblox Studio.
--========================================================--
...
PageHeader(
    ...
    "⌘",
    "Mountain Lab",
    "Cristais, jogadores, boulders, teleporte e testes."
)
...
local ScriptDescription =
    CreateLabel(
        ScriptInfo,
        "Painel funcional para o seu jogo no Roblox Studio.\n"
        .. "Detecta alvos por tags/nomes e procura remotes de coleta, mineração e venda.",
        12,
        GRAY,
        Enum.Font.Gotham
    )
...
local MountainVisual =
    Instance.new("Frame")

MountainVisual.BackgroundColor3 =
    PANEL
...
local MountainVisualPadding =
    Instance.new("UIPadding")

MountainVisualPadding.PaddingLeft =
    UDim.new(0, 11)

MountainVisualPadding.PaddingRight =
    UDim.new(0, 11)

MountainVisualPadding.PaddingTop =
    UDim.new(0, 11)

MountainVisualPadding.PaddingBottom =
    UDim.new(0, 15)

MountainVisualPadding.Parent =
    MountainVisual

local MountainVisualLayout =
    Instance.new("UIListLayout")

MountainVisualLayout.Padding =
    UDim.new(0, 8)

MountainVisualLayout.Parent =
    MountainVisual
...
CreateToggle(
    ...
    function(State)
        if not StudioMountainAllowed() then
            MountainState.CrystalESP = false
            return
        end

        MountainState.CrystalESP = State
        RefreshMountainHighlights()
    end
)
...
CreateToggle(
    ...
    function(State)
        if not StudioMountainAllowed() then
            MountainState.BoulderESP = false
            return
        end

        MountainState.BoulderESP = State
        RefreshMountainHighlights()
    end
)
...
local MountainMovement =
    Instance.new("Frame")

MountainMovement.BackgroundColor3 =
    PANEL

MountainMovement.Size =
    UDim2.new(
        1,
        -10,
        0,
...
local MountainMovementPadding =
    Instance.new("UIPadding")

MountainMovementPadding.PaddingLeft =
    UDim.new(0, 11)

MountainMovementPadding.PaddingRight =
    UDim.new(0, 11)

MountainMovementPadding.PaddingTop =
    UDim.new(0, 11)

MountainMovementPadding.PaddingBottom =
    UDim.new(0, 15)

MountainMovementPadding.Parent =
    MountainMovement

local MountainMovementLayout =
    Instance.new("UIListLayout")

MountainMovementLayout.Padding =
    UDim.new(0, 8)

MountainMovementLayout.Parent =
    MountainMovement

CreateToggle(
    MountainMovement,
    "Teleporte para cristais • Studio",
    false,
    function(State)
        if not StudioMountainAllowed() then
            MountainState.Teleport = false
            return
        end

        MountainState.Teleport = State
...
--========================================================--
-- FARM / AUTOMAÇÃO DE LABORATÓRIO
--========================================================--

local MountainFarm =
    Instance.new("Frame")

MountainFarm.BackgroundColor3 =
    PANEL
...
local MountainFarmPadding =
    Instance.new("UIPadding")

MountainFarmPadding.PaddingLeft =
    UDim.new(0, 11)

MountainFarmPadding.PaddingRight =
    UDim.new(0, 11)

MountainFarmPadding.PaddingTop =
    UDim.new(0, 11)

MountainFarmPadding.PaddingBottom =
    UDim.new(0, 15)

MountainFarmPadding.Parent =
    MountainFarm

local MountainFarmLayout =
    Instance.new("UIListLayout")

MountainFarmLayout.Padding =
    UDim.new(0, 8)

MountainFarmLayout.Parent =
    MountainFarm

CreateToggle(
    MountainFarm,
    "Farm de boulders • Studio",
    false,
    function(State)
        if not StudioMountainAllowed() then
            MountainState.Farm = false
            return
        end

        MountainState.Farm = State
...
CreateToggle(
    ...
    function(State)
        if not StudioMountainAllowed() then
            MountainState.Pickup = false
            return
        end

        MountainState.Pickup = State
...
CreateToggle(
    ...
    function(State)
        if not StudioMountainAllowed() then
            MountainState.Sell = false
            return
        end

        MountainState.Sell = State
...
CreateToggle(
    MountainFarm,
    "Anti-AFK • estado de teste",
    false,
    function(State)
        MountainState.AntiAFK = State

        Notify(
            "Mountain Lab",
            State
                and "Estado Anti-AFK registrado para testes."
                or "Estado Anti-AFK desativado."
        )
    end
)
    ...
    Connect(
        function(
            ...
            if not Humanoid or not Root then
                warn("CAFEÍNA V1: personagem respawnou sem Humanoid/Root prontos.")
                return
            end

...
local TeleportBehindDistance = 5.0
...
EconomySubtitle.Text = "SEU TYCOON"
...
EconomyHint.Text = "Compra os botões disponíveis do seu Tycoon e para quando nenhuma compra consegue ser concluída."
...
local function TriggerTycoonTouch(part)
    ...
    -- Quando disponível no ambiente de teste/executor, aciona exatamente
    -- o TouchTransmitter que o botão usa.
    ...
    -- Fallback: encosta fisicamente o personagem no botão.
    local original = character:GetPivot()
    local ok = pcall(function()
        character:PivotTo(part.CFrame + Vector3.new(0,2.5,0))
        task.wait(0.15)
    end)

    -- Tenta voltar para não deixar o Auto Buy arrastando o jogador pelo Tycoon.
    if ok and character.Parent and humanoid.Health > 0 then
        pcall(function()
...
local function IsNormalPurchaseButton(button, playerName)
    ...
    local price = tonumber(button:GetAttribute("Price"))
    if not price or price < 0 then
        return false
    end

    -- Gamepasses / botões marcados como NonPurchase não entram no comprar tudo.
    if button:GetAttribute("Gamepass") ~= nil then
        return false
    end
...
local function GetAvailablePurchaseButtons(tycoon)
    ...
    for _, button in ipairs(buttonsFolder:GetChildren()) do
        if IsNormalPurchaseButton(button, playerName) then
            table.insert(result, button)
        end
    end

    -- Mais baratos primeiro. Isso tende a liberar dependências gastando
    -- o saldo de forma progressiva.
...
local function DidPurchaseComplete(button, oldParent)
    if not button then
        return true
    end
    ...
    if oldParent and button.Parent ~= oldParent then
        return true
    end

    if not button:FindFirstChild("Touch") then
        return true
    end
...
local function StartAutoBuy()
    ...
    SetEconomyStatus("Procurando compras disponíveis...")
        ...
        while AutoBuying and myToken == AutoBuyToken and purchasedCount < maxPurchases do
            ...
            if #candidates == 0 then
                StopAutoBuy(
                    ...
                    or "Nenhuma compra normal disponível."
                )
                return
            end
            ...
            for _, button in ipairs(candidates) do
                ...
                if button.Parent and IsNormalPurchaseButton(button, LocalPlayer.Name) then
                    ...
                    if touch then
                        ...
                        if DidPurchaseComplete(button, oldParent) then

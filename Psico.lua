...
local function TeleportToPlayer(player)
    ...
    -- Cancela qualquer teleporte anterior.
    TeleportBusyToken = TeleportBusyToken + 1

    -- Zera apenas a inércia atual antes do salto.
    -- Não há loop nem aproximação gradual: o personagem é movido em uma única operação.
    localRoot.AssemblyLinearVelocity = Vector3.zero
    localRoot.AssemblyAngularVelocity = Vector3.zero

    -- Usa exatamente o CFrame do HumanoidRootPart do alvo.
    -- Não adicionamos deslocamento lateral/frontal, então ele pode ocupar a mesma posição.
    local destination = targetRoot.CFrame
    localCharacter:PivotTo(destination)

    -- Evita que a velocidade antiga continue depois do PivotTo.
    local newRoot = GetRoot(localCharacter)
    if newRoot then

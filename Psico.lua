...
--   • Estabilização curta contra retorno por física local
--
...
-- O botão da categoria abre um painel lateral independente.
-- Verde = aliado, vermelho = inimigo, cinza = time não identificado.
local TeleportBody = MakeAccordion("Teleport", "◎")

local TeleportHint = Instance.new("TextLabel")
TeleportHint.Size = UDim2.new(1,0,0,38)
TeleportHint.BackgroundTransparency = 1
TeleportHint.Text = "Abra a lista e toque no jogador para teleportar."
TeleportHint.TextColor3 = TEXT_MUTED
TeleportHint.TextSize = 9
TeleportHint.TextWrapped = true
TeleportHint.Font = Enum.Font.GothamMedium
TeleportHint.Parent = TeleportBody

local OpenTeleportListButton = MakeButton(TeleportBody, "ABRIR LISTA DE JOGADORES", 38)
...
local function TeleportToPlayer(player)
    ...
    -- PivotTo move o Model inteiro, e não só o HRP. Em seguida repetimos por
    -- alguns frames e zeramos a velocidade para evitar o "efeito elástico"
    -- causado por física/replicação local logo após o teleporte.
    task.spawn(function()
        local started = os.clock()
        while os.clock() - started < 0.9 do
            if myToken ~= TeleportBusyToken then return end
            ...
            if not currentCharacter or not currentHumanoid or currentHumanoid.Health <= 0 then return end
            if not currentTargetCharacter or not currentTargetHumanoid or currentTargetHumanoid.Health <= 0 or not currentTargetRoot then return end

            local destination = currentTargetRoot.CFrame * CFrame.new(3.5, 0, 1.5)
            currentCharacter:PivotTo(destination)
            StopCharacterMotion(currentCharacter)
            task.wait(0.05)
        end
    end)
end
